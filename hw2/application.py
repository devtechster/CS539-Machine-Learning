...
# Function to find the optimal alpha based on test loss
def function_alpha(Xtest, Ytest, Xtrain, Ytrain):
    no_of_epochs = 500
    listOfAlpha = []
    testLossList = []
    min_testLoss = 0
    for i in range(no_of_epochs):
        current_alpha = i / no_of_epochs
        listOfAlpha.append(current_alpha)
        w = train(Xtrain, Ytrain, current_alpha, no_of_epochs)
        testYhat = compute_yhat(Xtest, w)
        current_testloss = compute_L(testYhat, Ytest)
        testLossList.append(current_testloss)
    min_testLoss = min(testLossList)
    index = testLossList.index(min_testLoss)
    optimum_alpha = listOfAlpha[index]
    
    return optimum_alpha

# Function to find the optimal number of epochs based on test loss
def function_getEpochs(Xtest, Ytest, Xtrain, Ytrain, alpha):
    epochsList = []
    testLossList = []
    trainLossList = []
    Yhat = []
    ...
    for i in range(500):
        ...
        trainLossList.append(current_trainloss)
        testYhat = compute_yhat(Xtest, w)
        Yhat.append(testYhat)
        current_testloss = compute_L(testYhat, Ytest)
        testLossList.append(current_testloss)
    min_trainLoss = min(trainLossList)
    min_testLoss = min(testLossList)
    index = testLossList.index(min_testLoss)
...
# Function to get the relation between alpha, epochs, train loss, and test loss
...
# Finding optimal alpha and number of epochs
optimum_alpha = function_alpha(Xtest, Ytest, Xtrain, Ytrain)
optimum__epochs, min_trainLoss, min_testLoss, predicted_Yhat = function_getEpochs(Xtest, Ytest, Xtrain, Ytrain, optimum_alpha)
alphaList, epochsList, trainLoss, testLoss = get_alpha_epochs_relation(Xtrain, Ytrain, Xtest, Ytest)

# Printing results
# print(alphaList, epochsList, trainLoss, testLoss)
...
print('Minimum Train Loss: ', min_trainLoss)
#########################################