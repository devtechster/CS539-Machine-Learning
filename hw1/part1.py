...
#-------------------------------------------------------------------------------------------------------------------------------------
    ...
    def entropy(Y):
        ...
        listOfValues = Counter(Y)
        allValues = list(listOfValues.keys())
        total = len(Y)
        e=0
        for eachValue in allValues:
            p =  listOfValues[eachValue]/total
            e += -p * math.log2(p)
        #########################################
        return e
#--------------------------------------------------------------------------------------------------------------------------------------    
    '''
    Compute the conditional entropy of y given x. The conditional entropy H(Y|X) means average entropy of children nodes, given attribute X. 
    Refer to https://en.wikipedia.org/wiki/Information_gain_in_decision_trees
        Input:
            X: a list of values , a numpy array of int/float/string values. The size of the array means the number of instances/examples. 
            X contains each instance's attribute value. 
            Y: a list of values, a numpy array of int/float/string values. Y contains each instance's corresponding target label. 
            For example X[0]'s target label is Y[0]
        Output:
    ...
    def conditional_entropy(Y,X):
        #########################################
        ## INSERT YOUR CODE HERE
        allUniqueValuesOfX = np.unique(X)
        total = len(Y)
        ce=0
        for eachX in allUniqueValuesOfX:
            relY = Y[X==eachX]
            p =  len(relY)/total
...
#--------------------------------------------------------------------------------------------------------------------------------------    
    ...
    def information_gain(Y,X):
        #########################################
        ## INSERT YOUR CODE HERE
        informationGain = Tree.entropy(Y) - Tree.conditional_entropy(Y,X) 
        #########################################
...
#--------------------------------------------------------------------------------------------------------------------------------------
    ...
    def best_attribute(X,Y):
        ...
        for j in range(len(X)):
            ...
            attributeFunctionList.append(g)
        index = attributeFunctionList.index(max(attributeFunctionList))
        #########################################
        return index
#--------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def split(X,Y,i):
        ...
        #########################################
        ## INSERT YOUR CODE HERE
        splitFunctionAttribute = X[i]
        uniqueAttribute = np.unique(splitFunctionAttribute)
        attrAndChildrenDictionary=dict()
        for singleAttribute in uniqueAttribute:
            relY = Y[splitFunctionAttribute==singleAttribute]
            relX = X[:,splitFunctionAttribute==singleAttribute]
            attrAndChildrenDictionary[singleAttribute] = Node(relX,relY)
        #########################################
        return attrAndChildrenDictionary
#--------------------------------------------------------------------------------------------------------------------------------------
    @staticmethod
    def stop1(Y):
        '''
...
#--------------------------------------------------------------------------------------------------------------------------------------   
    ...
    def stop2(X):
        ...
        for iterator in range(len(X)):
            state = Tree.stop1(X[iterator])
            statesList.append(state)
        c = list(Counter(statesList).keys())
