...
def compute_L(a,y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    small_epsilon_val = 1e-15  # Small epsilon to avoid taking the logarithm of zero or one
    aValue = np.clip(a, small_epsilon_val, 1 - small_epsilon_val)  # Clip predicted probabilities
    # Handle cases where the loss is too large
    L = -np.sum(y * np.log(aValue) + (1 - y) * np.log(1 - aValue))
    max_loss = 1e7  # Set a predefined maximum value for the loss
    if L > max_loss:
        L = max_loss
    	
    # Additional check to set a large loss when a is exactly 1 and y is 0
